...
GenerateMove( Files * files, DistParms * distp, ScoreOutput * out ) {
    ...
    /*===========
        AMIR: It's only true for the first loop.
        */
    if( old_energy == -999. ) {
    ...
    /*=============
        Amir: it only change one paramters at a time.

        1. Chane one paramters
        */
    Move( files, distp );       
    ...
    /*====================
        Amir: Compute the objective function.                                
              distp uses for SA.
...
UpdateControl( Files * files ) {
...
#endif
        if( prolix ) {
            outfile = fopen( files->prolixfile, "a" );
            if( !outfile )
    ...
    for( i = 0; i < nparams; i++ ) {
        ...
        if( prolix ) {
            ...
            if( myid == 0 ) {
#endif
                fprintf( outfile, "nsteps = %8d index = %3d bar = %10.8e hits = %4d ", nhits, i, acc_tab[i].theta_bar, acc_tab[i].hits );
 ...
 */
char *
SetProlix( int value, char *file, int init_flag ) {
    FILE *prolixptr;
    char *prolix_filename;

    const char *suffix = ".prolix";     /* suffix for prolix file */

    prolix_filename = ( char * ) calloc( MAX_RECORD, sizeof( char ) );
    prolix_filename = strcpy( prolix_filename, file );
    prolix_filename = strcat( prolix_filename, suffix );

    if( init_flag ) {           /* this deletes an old .prolix file if present */
        prolixptr = fopen( prolix_filename, "w" );
        if( !prolixptr )
...
RestoreProlix( char *prolixfile ) {
    ...
    if( myid == 0 ) {           /* only root restores prolix */
        ...
        while( ( saved_nsteps <= nhits ) && ( NULL != fgets( prolixline, MAX_RECORD, prolixptr ) ) ) {
            if( 1 != sscanf( prolixline, "nsteps = %d", &saved_nsteps ) )
                error( "RestoreProlix: error reading saved_nsteps (after %d)", saved_nsteps );
        ...
        if( -1 == system( shell_cmd ) )
            error( "RestoreProlix: error renaming temp file %s", outfile );

        if( remove( outfile ) )
            warning( "RestoreProlix: temp file %s could not be deleted", outfile );
