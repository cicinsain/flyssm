 ...
 *
 *
 *   Amir: With a little modification, it reports the score for each genes.
 *   Amir: TODO: Add #ifdef for different types of objectives function...
 *   Amir: TODO: How to deal with out->penalty
 */
void
Score( Input * inp, ScoreOutput * out, int jacobian ) {
    ...
    /*======
        Amir: To store each objectives chisq.*/
    #if defined(AMOSA) || defined(NSGA2)
        double *chisq_s;
    ...
    /*==== 
        Amir: Initialize the struct for each genes.*/
    ...
    // printf("%f\n", out->score);
        ...
        for (int g = 0; g < inp->zyg.defs.ngenes; ++g){
            // out->nsga2_outs[g]->score = chisq_s[g];
            out->nsga2_outs[g]->score = chisq_s[g] + out->penalty;
            // printf("%f,", out->nsga2_outs[g]->score);
        }
...
#if defined(AMOSA) || defined(NSGA2)
/*
    GeneBasedEval: Perform the same function of Eval(), and also reports the chisq for each genes.

*/
void 
GeneBasedEval(ScoreEval * eval, NArrPtr * Solution, int gindex, Input * inp )
{
    ...
    double *difference_s;         
        difference_s = (double *) malloc ( inp->zyg.defs.ngenes * sizeof(double) );
    double *chisq_s;           
        chisq_s      = (double *) calloc ( inp->zyg.defs.ngenes,  sizeof(double) );
    int *currsize_s;
        currsize_s = (int *) calloc (inp->zyg.defs.ngenes ,  sizeof(int) );
    ...
    // printf("----%d\n", fact_tab.size);
    /*====
        Amir: I can actually control the size of the number of time-point that I'm going to optimize by modyfing this loop.*/
    ...
    for( tindex = 0; tindex < fact_tab.size; tindex++ ) 
    {
        ...
        /*=================
            Amir: Increase the sindex to achieve the correct index fo solution. */
        ...
        /*====
            Amir: No need to do this, because I couldn't find any code working with residuals.*/
        // for (int g = 0; g < inp->zyg.defs.ngenes; ++g){
        //     eval->nsga2_evals[g].residuals = ( double * ) realloc( eval->nsga2_evals[g].residuals, ( currsize[g] + fact_tab.record[tindex].size ) * sizeof( double ) );
        // }
        ...
        for( vindex = 0; vindex < fact_tab.record[tindex].size; vindex++ ) {
            ...
            /*=====
                Amir: Computing the differences.*/
            difference_s[oindex] = point.conc - v[point.index];
            ...
            /*===============
                Amir: getting chisqs
                */
            chisq_s[oindex] += difference_s[oindex] * difference_s[oindex];
            ...
            /*==============
                Amir: Collecting the residuals.
                */
                // TODO: Initialize the nsga2_evals
                // TODO: Shall I have more than one currsize!?
                // for (int g = 1; g <= inp->zyg.defs.ngenes; ++g){
                //     eval->nsga2_evals.residuals[currsize - 1] = fabs(difference_s[g-1]);
                // }                
    ...
    // printf("eval->residuals_size: %d\n", eval->residuals_size);
    // printf("inp->zyg.ndp: %d", inp->zyg.ndp);


    /*================
        Amir: eval->nsga2_evals is initialized in Blastoderm()
        */
        // printf("%d\n", inp->zyg.defs.ngenes);
        // printf("%d\n", inp->zyg.defs.egenes);
