...
void realcross (NSGA2Type *nsga2Params,  individual *parent1, individual *parent2, individual *child1, individual *child2)
{
    ...
    if (randomperc() <= nsga2Params->pcross_real)   // nsga2Params.pcorss_real = p_c 
    {
        nsga2Params->nrealcross++;
        for (i=0; i<nsga2Params->nreal; i++)    
        {
            // The value 0.5 corresponds to the probablity of applying the mutation operator
            // on every decision variable, p_ic.
            if (randomperc()<=0.5 ) 
            {
                if (fabs(parent1->xreal[i]-parent2->xreal[i]) > EPS)
                {
                    ...
                    if (c2>yu)
                        c2=yu;
                    if (randomperc()<=nsga2Params->p_ic)
                    {   // The probablity of swapping child values p_e
                        child1->xreal[i] = c2;
                        child2->xreal[i] = c1;
                    }
                    else
